...
<div id=subitems>',
  ...
  subitems: '
    <span class="subitem">
      <a href="{subitem-path}" download="{subitemfilename}">
...
def data_uri(filename)
  data = Base64.encode64(File.read(filename))
  mime = Open3.capture2('/usr/bin/file', '--mime-type', '-b', filename).first.strip
...
categories.each do |category_dir|
  ...
  category_img = data_uri(Dir.glob(File.join(category_path, "thumb.{png,jpg,jpeg,gif}")).first||"nothumb.svg")
...
categories.each do |category_dir|
  ...
  items.each do |item_dir|
    ...
    item_img = data_uri(Dir.glob(File.join(item_path, "thumb.{png,jpg,jpeg,gif}")).first||"nothumb.svg")
...
categories.each do |category_dir|
  ...
  items.each do |item_dir|
    ...
    subitems.each do |subitem_file|
      subitem_path = data_uri(File.join(item_path,subitem_file))
      # get subitem info
      data = subitem_file.match(gcode_name_parser)
      subitem_img = data_uri(Dir.glob(File.join(item_path, data[:name]+".{png,jpg,jpeg,gif}")).first||"nothumb.svg")
      notes = [
        data[:name],
